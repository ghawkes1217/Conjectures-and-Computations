...
#such that no two adjacent entries are equal
...
#Given a list replaces each entry with its absoute value.
def pos(w):
    p=[]
    for i in range(0,len(w)):
        p.append(abs(w[i]))
...
# Consecutive entries may not be added to the same box.
def children(par):
    C=[]
    p=pos(par)
...
#with no consecutive entries in the same box
def createT(length):
    Sizes=[[],[[-1]]]
...
def all_check(length,largest):
    SH=createT(length)
    T=distabs(copy.deepcopy(SH[length]))  
