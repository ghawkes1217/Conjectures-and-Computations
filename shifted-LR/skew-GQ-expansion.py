        ...
        if same_shape==True:
            ...
            for i in range(0,len(positions)-1):
                if positions[i]==positions[i+1]:
                    repeat_set.append(i)
            P.append([tab[0],peak_set,repeat_set])
...
def first(w):
    #check that first i or i' appearing in read_w is i
    #(this is the difference between GR and GQ (GP is different than both of these))
    maxi=max(w)
    if maxi%2==1:
        return(False)
        #because then first instance of (maxi+1)/2 is primed
    if maxi%2==0:
        starts=[0]*int(maxi/2)

        for j in range(0,len(w)):
            base=math.ceil(w[j]/2)
            #base is the i we are checking
            if starts[base-1]==0:
                #is this is the first i or i' appearing?
                if w[j]%2==0:
                    starts[base-1]=1
                    #record that the first i or i' is an i
                if w[j]%2==1:
                    starts[base-1]=-1
                    #record that the first i or i' is an i'
        ok=True
...
def GR_expand(skew,shape):
    ...
    for i in range(0,len(F)):
        ...
        if first(read_w(tab))==True and latt(primed_start(tab))==True:
            W.append(weights(read_w(tab)))
            #Record the weights of these tableaux
            if weights(read_w(tab))[0:3]==[554,3,0]:
                print(tab)
